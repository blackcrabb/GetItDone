...
// saveData("TotalTasks", 0);
    ...
    readTask(taskStore , function(tasks) {
       ...
       for(let i = 0;i< tasks.length ; i++){
           innerHTML += `
           <li data-id='${tasks[i].id}' onclick='deleteOnClick(this)'>
            ${tasks[i].title}
           </li>
           `;
    ...
    //Math.min(tasks.length, maxRecentlyDeleted)  tasks.length<4 ? tasks.length : 4
...
function updateTheme(theme){
    ...
    let shadColor = theme == 'light' ? "0, 0, 0" : "255, 255, 255" ;
    let grad1 = theme == 'light' ? "108, 29, 103" : "34, 208, 163" ;
    let grad2 = theme == 'light' ? "100, 25, 148" : "32, 173, 211" ;
    let sidebar1 = theme == 'light' ? "255,255,255" : "35, 35, 35" ;
    let sidebar2 = theme == 'light' ? "251, 247, 247" : "46, 46, 46" ;

    let root = document.documentElement;

    root.style.setProperty("--bg-color",bgColor);
    root.style.setProperty("--text-color",textColor);
    root.style.setProperty("--shadow-color",shadColor);
    root.style.setProperty("--gradient-1",grad1);
    root.style.setProperty("--gradient-2",grad2);
    root.style.setProperty("--sidebar-gradient-1",sidebar1);
    root.style.setProperty("--sidebar-gradient-2",sidebar2);
